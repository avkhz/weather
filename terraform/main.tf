...
resource "local_file" "tf_key_name" {
  content  = tls_private_key.rsa-2048.private_key_pem
  filename = var.file_name
}

resource "local_file" "tf_key_pem" {
  content  = tls_private_key.rsa-2048.public_key_openssh
  filename = var.pub_key_name
}
...
#Adding security group
 ...
 #Creating EKS Cluster
  ...
  resource "aws_eks_node_group" "node-grp" {
    ...
    disk_size       = 20
    instance_types  = [var.worker_node_ec2_type]
